...
- Paper snapshot replay/report windows should use match `kickoff_time` as the user-facing time axis. For historical periods without original snapshots, prefer `snapshot_source=historical_backfill`; for new operating windows from July 2026 onward, review `automation` as the primary real-use source and `manual_record` as a separately reported supplement. Do not treat an empty historical-backfill view as no activity or create duplicate backfills merely to populate the default page. Check cross-source group-key overlap before presenting a combined result. Zero-stake groups stay in flat-profit/flat-ROI review by default, while weighted metrics naturally exclude them through `suggested_stake_units=0.00`.
- Formal paper-snapshot reviews run on the first day of each month and lightweight reviews on the fifteenth. The 2026-07-16 half-month baseline and the 2026-08-01 prompt are recorded under `docs/交接/`.
- The paper tracking page's `confidence_simulation` should prefer frozen `paper_recommendation_group_snapshots` over dynamic confidence recomputation, using source priority `historical_backfill > manual_record > automation`; only groups without an exact snapshot fall back to dynamic computation.
