...
- Paper snapshot replay/report windows should use match `kickoff_time` as the user-facing time axis. Retrospective review should prefer `snapshot_source=historical_backfill` to avoid double-counting real-time `automation` snapshots. Zero-stake groups stay in flat-profit/flat-ROI review by default, while weighted metrics naturally exclude them through `suggested_stake_units=0.00`.
