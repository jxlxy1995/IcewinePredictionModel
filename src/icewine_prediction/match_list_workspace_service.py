...
def build_match_list_workspace(
    ...
    *,
    ...
    start, end = _time_window(now, time_preset)
    ...
    if status_filter != "all":
        query = query.filter(Match.status.in_(_statuses_for_filter(status_filter)))
    ...
    return MatchListWorkspace(
        ...
        leagues=_league_options(session),
...
def _match_row(
    ...
    *,
    ...
    return MatchListRow(
        ...
        status=match.status,
        status_group=_status_group(match.status),
...
def _league_options(session: Session) -> list[str]:
    rows = session.query(Match).options(joinedload(Match.league)).all()
    return sorted({match.league.name for match in rows})
...
def _time_window(now: datetime, preset: str) -> tuple[datetime | None, datetime | None]:
    if preset == "next_24h":
        return now, now + timedelta(hours=24)
    if preset == "next_3d":
        return now, now + timedelta(days=3)
    if preset == "previous_24h":
        return now - timedelta(hours=24), now
    if preset == "previous_7d":
...
def _statuses_for_filter(status_filter: str) -> list[str]:
    if status_filter == "not_started":
        return ["scheduled", "not_started"]
    if status_filter == "live":
        return ["live", "in_play", "halftime"]
    if status_filter == "finished":
        return ["finished"]
    return ["scheduled", "not_started", "live", "in_play", "halftime", "finished"]
