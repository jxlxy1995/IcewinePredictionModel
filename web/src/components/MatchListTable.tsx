...
export function MatchListTable({ onOpenMatch, workspace }: MatchListTableProps) {
      ...
      <tbody>
        {rows.map((row) => (
          ...
          >
            ...
            <td>{row.league}</td>
            <td>
              <TeamName logoUrl={row.match.home_team_logo_url} name={row.match.home_team_display_name ?? row.match.home_team_name} />
              <span className="versus-text">vs</span>
              <TeamName logoUrl={row.match.away_team_logo_url} name={row.match.away_team_display_name ?? row.match.away_team_name} />
            </td>
